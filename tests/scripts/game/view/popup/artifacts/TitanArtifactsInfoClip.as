package game.view.popup.artifacts
{
   ...
   public class TitanArtifactsInfoClip extends GuiClipNestedContainer implements ITooltipSource
   {
      ...
      public var mediator:TitanArtifactsPopupMediator;
      
      private var _tooltipVO:TooltipVO;
      
      public function TitanArtifactsInfoClip()
      {
         ...
         layout_desc = ClipLayout.verticalMiddleCenter(5,tf_desc,info_icon);
      ...
      {
         TooltipHelper.removeTooltip(info_icon.graphics);
      }
      ...
      public function updateState(param1:Boolean = false, param2:Boolean = false) : void
      {
         ...
         if(mediator.artifact)
         {
            ...
            updateStats();
            _loc6_ = AssetStorageUtil.getTitanArtifactBigFrameTexture(mediator.artifact);
            if(_loc3_)
            {
            ...
            info_icon.graphics.visible = mediator.artifact.desc.artifactType == "armor" || mediator.artifact.desc.artifactType == "weapon";
            if(mediator.artifact.desc.artifactType == "armor")
            {
               _tooltipVO.hintData = Translate.translate("UI_DIALOG_TITAN_ARTIFACTS_ARMOR_DESC");
            }
            else if(mediator.artifact.desc.artifactType == "weapon")
            {
               _tooltipVO.hintData = Translate.translate("UI_DIALOG_TITAN_ARTIFACTS_WEAPON_DESC");
            }
            item_image.image.texture = AssetStorage.rsx.titan_artifact_icons_large.getTexture(mediator.artifact.desc.assetTexture);
         }
      }
      ...
      override public function setNode(param1:Node) : void
      {
         ...
         TooltipHelper.addTooltip(info_icon.graphics,tooltipVO);
      }
